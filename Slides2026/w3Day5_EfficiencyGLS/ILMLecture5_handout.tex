...
\begin{frame}{GLS Through the Same Lens}
\begin{itemize}
...
\item \textbf{Mechanism:} The same linear model $Y = X'\beta + e$, but now we claim to know \emph{which observations are noisier than others}.\pause
...
\item \textbf{Moment condition:}
$$\mathbb{E}[\Sigma^{-1/2}X_i(\Sigma^{-1/2}Y_i - \Sigma^{-1/2}X_i'\beta)]=0$$
Weight observations inversely to their noise.\pause

\item \textbf{Estimator:} $\hat{\beta}_{GLS}=(\bm{X}'\Sigma^{-1}\bm{X})^{-1}\bm{X}'\Sigma^{-1}\bm{Y}$.

\item The claim ``I know $\Sigma$'' is itself a substantive assumption---if wrong, GLS can be worse than OLS.
\end{itemize}
\end{frame}
